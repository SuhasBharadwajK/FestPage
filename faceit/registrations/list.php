<?php
  ...
  //$resultArray = array();
  $resultTable = "";
  if (mysqli_num_fields($result) == 11) {
    # code...
    echo "<tr><th>ID</th><th>Name</th><th>Email</th><th>Year</th><th>Branch</th><th>Roll Number</th><th>College</th><th>Phone Number</th><th>Referral Code</th><th>List of Events</th><th>Mobile?</th></tr>";
    while($row = $result->fetch_assoc()) {
      echo "<tr><td>{$row['id']}</td><td>{$row['name']}</td><td>{$row['email']}</td><td>{$row['year']}</td><td>{$row['branch']}</td><td>{$row['rollnum']}</td><td>{$row['college']}</td><td>{$row['phonenum']}</td><td>{$row['refcode']}</td><td>{$row['events']}</td><td>{$row['mobile']}</td></tr>";
    }
  }
